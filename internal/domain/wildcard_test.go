...
func TestNormalizeWildcardURL(t *testing.T) {
	...
	tests := []struct {
		...
		{"legacy-json-query", "logs/*", `https://g.example.com/explore?left={"q":"up"}&id=*`, `https://g.example.com/explore?left={"q":"up"}&id=*`, false},
		{"legacy-literal-name-braces", "pulls/*", "https://example.com/{a}/*", "https://example.com/%7Ba%7D/*", false},
		{"legacy-no-star-but-braces", "pulls/*", "https://example.com/{a}", "", true},
